...
const GlossaryManager: React.FC<GlossaryManagerProps> = ({ glossary, addTerm, removeTerm, updateTerm, activeGlossaryTerms }) => {
  ...
  const paginatedGlossary = useMemo(() => {
    ...
    return filteredGlossary.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  }, [filteredGlossary, currentPage]);
  ...
  const handleStartEdit = (entry: GlossaryEntry) => {
    ...
    setEditForm({ original: entry.original, translation: entry.translation });
  };
  ...
  return (
    <div className="bg-[var(--bg-secondary)] rounded-md p-4 flex flex-col border border-[var(--border-primary)]">
      <h2 className="text-lg font-bold text-[var(--accent-primary)] mb-4">Glossary</h2>
      <div className="w-full overflow-hidden">
        <form onSubmit={handleAddTerm} className="flex gap-2 mb-4">
          <input
            ...
            className="flex-1 bg-[var(--bg-tertiary)] border border-[var(--border-primary)] rounded-md px-3 py-2 text-sm text-[var(--text-primary)] focus:ring-2 focus:ring-[var(--accent-primary)] focus:border-[var(--accent-primary)] outline-none transition"
          />
          <input
            type="text"
            value={translation}
          ...
          <button
            type="submit"
            className="bg-[var(--accent-primary)] hover:bg-[var(--accent-secondary)] text-[var(--text-on-accent)] font-semibold px-4 py-2 rounded-md flex items-center justify-center transition-colors disabled:bg-gray-600 disabled:text-gray-400"
      ...
      <div>
        {glossary.length === 0 ? (
          <p className="text-[var(--text-secondary)] text-sm text-center py-4">Your glossary is empty. Add terms above.</p>
        ) : filteredGlossary.length === 0 ? (
          <p className="text-[var(--text-secondary)] text-sm text-center py-4">No terms match your search.</p>
        ) : (
          <>
            <ul className="space-y-2">
              {paginatedGlossary.map((entry) => {
                const isActive = activeGlossaryTerms.some(activeTerm => activeTerm.original === entry.original);
                return (
                  <li
                    key={entry.original}
                    className="flex justify-between items-center bg-[var(--bg-tertiary)] p-2 rounded-md text-sm group"
                  >
                    {editingOriginal === entry.original ? (
                      <form onSubmit={handleUpdateTerm} className="flex-1 flex items-center gap-2">
                        <input
                          type="text"
                          value={editForm.original}
                          onChange={(e) => setEditForm(prev => ({ ...prev, original: e.target.value }))}
                          className="flex-1 bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded px-2 py-1 text-sm text-[var(--text-primary)] focus:ring-1 focus:ring-[var(--accent-primary)] outline-none"
                          aria-label="Edit original term"
                        />
                        <input
                          type="text"
                        ...
                        <div className="flex items-center gap-1">
                          <button type="submit" className="text-[var(--text-secondary)] hover:text-[var(--success-primary)]" aria-label="Save changes"><CheckIcon /></button>
                          <button type="button" onClick={handleCancelEdit} className="text-[var(--text-secondary)] hover:text-[var(--danger-primary)]" aria-label="Cancel edit"><XIcon /></button>
                        </div>
                      </form>
                    ) : (
                      <>
                        <div className="flex items-center gap-2">
                          {isActive && (
                            <span title="This entry is present in the original text and will be sent to the AI.">
                              <ActiveIcon />
                            </span>
                          )}
                          <div>
                            <span className="font-semibold text-[var(--text-primary)]">{entry.original}</span>
                            <span className="text-[var(--text-secondary)] mx-2">→</span>
                            <span className="font-semibold text-[var(--accent-primary)]">{entry.translation}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() => handleStartEdit(entry)}
                            className="text-[var(--text-secondary)] hover:text-[var(--accent-secondary)]"
                            aria-label={`Edit ${entry.original}`}
                          >
                            <PencilIcon />
                          </button>
                          <button
                            onClick={() => removeTerm(entry.original)}
                            className="text-[var(--text-secondary)] hover:text-[var(--danger-secondary)]"
                            aria-label={`Remove ${entry.original}`}
                          >
                            <TrashIcon />
                          </button>
                        </div>
